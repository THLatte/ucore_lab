...
alloc_proc(void) {
     ...
     */
     ...
     //LAB5 2012011275 : (update LAB4 steps)
    /*
	 ...
	 */
     ...
     //LAB6 2012011275 : (update LAB5 steps)
    /*
    ...
    //LAB8:EXERCISE2 2012011275 HINT:need add some code to init fs in proc_struct, ...
     proc->filesp = NULL;
    }
    return proc;
}
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    ret = -E_NO_MEM;
    //LAB4:EXERCISE2 2012011275
    //LAB8:EXERCISE2 2012011275  HINT:how to copy the fs in parent's proc_struct?
    /*
    ...
    local_intr_save(intr_flag);
    {
        proc->pid = get_pid();
        hash_proc(proc);
        //因为set_links函数中包含了将proc插入到proc_list的操作和增加进程计数的操作，故将这两个操作删除
        //list_add(&proc_list, &(proc->list_link));
        //nr_process ++;
        set_links(proc);
        //cprintf("\nNew proc added. Now we have %d processes.\n\n", nr_process);
    }
    local_intr_restore(intr_flag);
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2012011275  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
